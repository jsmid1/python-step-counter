...
PyObject* tuple_contains_patched_method;
...
static PyObject *patched_tuple_getitem(PyObject *self, PyObject* elem) {
    PyObject* result = PyObject_CallFunction(tuple_getitem_patched_method, "(OO)", self, elem);

    return result;
}

static int patched_tuple_contains(PyObject *self, PyObject* elem) {
    PyObject* result = PyObject_CallFunction(tuple_contains_patched_method, "(OO)", self, elem);
...
static PyObject* patch_tuple_contains(PyObject* self, PyObject* args) {
    PyObject* patched_method;
    if (!PyArg_ParseTuple(args, "O", &patched_method)) {
        return NULL;
    }
    ...
    tuple_contains_patched_method = patched_method;

    PyTuple_Type.tp_as_sequence->sq_contains = patched_tuple_contains;
...
static PyMethodDef patchtuple_methods[] = {
    ...
    {"patch_tuple_contains", patch_tuple_contains, METH_VARARGS, "Set the method for tuple.__contains__ calls."},
