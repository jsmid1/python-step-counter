...
class TestTurtleMethods(unittest.TestCase):
    ...
    def test_os_remove(self):
        ...
        with recording_activated():
            os.remove(test_file)
        self.assertTrue(is_recorded(self.recorder, os, 'remove'))

    def test_os_getcwd(self):
        with recording_activated():
            os.getcwd()
        self.assertTrue(is_recorded(self.recorder, os, 'getcwd'))
    ...
    def test_os_mkdir(self):
        ...
        with recording_activated():
            os.mkdir(test_dir)
        os.rmdir(test_dir)
        self.assertTrue(is_recorded(self.recorder, os, 'mkdir'))

    def test_os_listdir(self):
        with recording_activated():
            os.listdir('.')
        self.assertTrue(is_recorded(self.recorder, os, 'listdir'))

    def test_os_makedirs(self):
        dir_path = 'newdir/nesteddir'
        with recording_activated():
            os.makedirs(dir_path)
        self.assertTrue(is_recorded(self.recorder, os, 'makedirs'))
        os.removedirs(dir_path)

    def test_os_chdir(self):
        with recording_activated():
            os.chdir('.')
        self.assertTrue(is_recorded(self.recorder, os, 'chdir'))

    def test_os_scandir(self):
        with recording_activated():
            with os.scandir('.') as _:
                pass
        self.assertTrue(is_recorded(self.recorder, os, 'scandir'))

    def test_os_rmdir(self):
        os.mkdir('test_dir')
        with recording_activated():
            os.rmdir('test_dir')
        self.assertTrue(is_recorded(self.recorder, os, 'rmdir'))
    ...
    def test_os_rename(self):
        ...
        self.assertTrue(is_recorded(self.recorder, os, 'rename'))
